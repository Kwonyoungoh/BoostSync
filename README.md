...
- **C++**: 이 프로젝트의 주요 언어입니다. 시스템 자원을 효율적으로 활용하고 복잡한 네트워크 프로그래밍에 적합합니다.
...
서버와 클라이언트 간의 메시지 전달은 Redis의 Pub/Sub 기능을 통해 이루어집니다. 서버는 `sub_channel`, `pub_msg`, `unsub_channel` 메서드를 통해 이를 처리하며, `handle_msg` 메서드를 통해 받은 메시지를 해당 청크의 모든 클라이언트에게 전달합니다.